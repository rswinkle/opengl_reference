...
vec3 uPntLightSpec(5);
vec3 uPntLightDiff(0.8);
int uUseLighting = 1;
int uUseSpecularMap = 1;
int uUseColorMap = 1;
int uColorMapSampler;
int uSpecularMapSampler;
...
int uUseSpecularMap_loc;
int uUseColorMap_loc;
int uColorMapSampler_loc;
int uSpecularMapSampler_loc;
...
int main(int argc, char** argv)
{
	...
	if (!load_texture2D("../media/textures/earth-specular.gif", GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR, GL_REPEAT, GL_TRUE, GL_FALSE)) {
		printf("failed to load texture\n");
		return 0;
	}


	const char vs_file[] = "../media/shaders/lesson15.vp";
	const char fs_file[] = "../media/shaders/lesson15.fp";
	...
	uUseSpecularMap_loc = glGetUniformLocation(program, "uUseSpecularMap");
	uUseColorMap_loc = glGetUniformLocation(program, "uUseColorMap");
	uSpecularMapSampler_loc = glGetUniformLocation(program, "uSpecularMapSampler");
	uColorMapSampler_loc = glGetUniformLocation(program, "uColorMapSampler");

	printf("%d %d\n", uSpecularMapSampler_loc, uColorMapSampler_loc);
	//assert(uSpecularMapSampler_loc >= 0 && uColorMapSampler_loc >= 0);


	//glActiveTexture(GL_TEXTURE0);
	//glBindTexture(GL_TEXTURE_2D, earth_tex);
	glUniform1i(uColorMapSampler_loc, 0);
	//set_uniform1i(program, "uColorMapSampler", 0);
	//glActiveTexture(GL_TEXTURE1);
	//glBindTexture(GL_TEXTURE_2D, specular_tex);
	glUniform1i(uSpecularMapSampler_loc, 1);
	...
	while (1) {
		...
		uMVMatrix = glm::rotate(uMVMatrix, glm::radians(earthAngle), vec3(0, 1, 0));
		...
		glUniform1i(uUseColorMap_loc, uUseColorMap);
		glUniform1i(uUseSpecularMap_loc, uUseSpecularMap);
		    ...
		    NK_WINDOW_MINIMIZABLE|NK_WINDOW_TITLE))
		{
			...
			if (nk_checkbox_label(ctx, "Use color map", &uUseColorMap)) {
				printf("Color Map %s\n", (uUseColorMap ? "On" : "Off"));
			}
			if (nk_checkbox_label(ctx, "Use specular map", &uUseSpecularMap)) {
				printf("Specular Map %s\n", (uUseSpecularMap ? "On" : "Off"));
			}
...
void setup_context()
{
	...
	window = SDL_CreateWindow("Lesson 15", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, SDL_WINDOW_OPENGL);
	if (!window) {
		cleanup();
		exit(0);
	}
